...
def get_cropIndices(shape, random_crops):
    """
    Get the indices for random crops of the data with shape random_crops.
    shape is the shape of input images
    random_crops is the size of the output crops
    in the returned variables, the first dim is for x and the second is y
    
    How it works:
        upo_down and left_right are bools that determine whether we get the
        crop form the end of the image or the beginning. Based on that, we
        take the crops of size random_crops
    """
...
class SegmentationDataGenerator(object):
    ...
    def random_transform(self, x, y):
        ...
        if self.random_crops:
            start, stop = get_cropIndices(x.shape[:2], self.random_crops)
...
class NumpyArrayIterator(Iterator):
    ...
    def next(self):
        ...
        if self.image_data_generator.nb_inputs:
            inputs = []
        ...
        if self.image_data_generator.nb_outputs:
            outputs = []
