...
#%%
def run_caiman(fnames):
    
#%% Select file(s) to be processed (download if not present)
    folder = os.path.split(fnames)[0]
    name = os.path.split(fnames)[1]
...
# %% compare with original movie
    if display_images:
        ...
        moviehandle.play(fr=300, q_max=99.5, magnification=2)  # press q to exit
...
# %%  parameters for source extraction and deconvolution
    ...
    merge_thr = 0.90         # merging threshold, max correlation allowed
    rf = 15
...
# %% RE-RUN seeded CNMF on accepted patches to refine and perform deconvolution
    ...
    min_SNR = 0.2  # signal to noise ratio for accepting a component
    ...
    #%%
    #idx=np.append(cnm2.estimates.idx_components, 36)
    idx = 161
    plt.figure();plt.plot(cnm2.estimates.C[idx])
    
    plt.plot(signal_filter(cnm2.estimates.C, freq=15, fr=300)[idx])
    plt.figure();plt.imshow(cnm2.estimates.A[:,idx].toarray().reshape(cnm2.dims, order='F'))
    #%%
    idx = 1
    plt.figure();plt.plot(cnm2.estimates.f[idx])
    
    #plt.figure();plt.plot(signal_filter(cnm2.estimates.C, freq=15, fr=400)[idx])
    plt.figure();plt.imshow(cnm2.estimates.b[:,idx].reshape(cnm2.dims, order='F'))
    
    
    #%%
    cnm2.estimates.idx = idx
    #%%
    ...
    np.save(f'{folder}/caiman_{name[:-5]}', estimates)
